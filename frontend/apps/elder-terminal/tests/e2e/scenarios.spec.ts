...
test.describe('场景 2 · CareSafe 烟雾/燃气安全告警', () => {
  test('不可忽略安全卡 → 安全页 → 查看时间与来源 → 无关闭/取消', async ({ page }, testInfo) => {
    ...
    await expect(page.getByText('烟雾/燃气')).toBeVisible();
    await expect(page.getByRole('heading', { name: '紧急求助' })).toBeVisible();
...
test.describe('场景 3 · CareRadar 低质量活动数据', () => {
  test('今日页活动待确认 → 时间线 quality 原因', async ({ page }, testInfo) => {
    ...
    // 今日页出现活动回顾任务，且不把 UNKNOWN 伪装成已完成
    await expect(page.getByText('活动回顾')).toBeVisible();
    await expect(page.getByRole('link', { name: /活动回顾/ })).toContainText('暂未确认');
...
test.describe('场景 4 · CareAgent DAILY_SUMMARY', () => {
  test('逐条展示事实与来源，AI 身份标识可见', async ({ page }, testInfo) => {
    ...
    await expect(page.getByText('CareDose 合成事件')).toBeVisible();
