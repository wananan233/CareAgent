...
describe('MockCoreAdapter 各结果状态', () => {
  ...
  it('空输入对话返回 fallback（带 reasonCode）', async () => {
    const api = new MockCoreAdapter();
    const r = await api.chat(DEMO_SUBJECT_ID, '   ');
    expect(r.ok).toBe(true);
    if (r.ok) {
      expect(r.data.fallback).toBe(true);
      expect(r.data.reasonCode).toBe('AGENT_FALLBACK');
    }
  });
  ...
  it('offline 故障返回 OFFLINE/NETWORK_OFFLINE 信封', async () => {
    ...
    if (!r.ok) {
      ...
      expect(r.error.error.code).toBe('OFFLINE');
      expect(r.error.error.reasonCode).toBe('NETWORK_OFFLINE');
    }
  });

  it('denied 故障返回 DENIED/SUBJECT_MISMATCH', async () => {
    const api = new MockCoreAdapter({ fault: 'denied' });
    const r = await api.getTasks(DEMO_SUBJECT_ID);
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.error.error.code).toBe('DENIED');
  });

  it('failed 故障返回 FAILED/UPSTREAM_FAILED 且可重试', async () => {
...
describe('幂等请求与版本控制', () => {
  it('同一 idempotency_key 重复提交只生效一次（版本仅 +1）', async () => {
    ...
    if (r1.ok && r2.ok) {
      expect(r1.data.status).toBe('RECEIVED');
      expect(r2.data.status).toBe('RECEIVED');
      expect(r2.data.commandId).toBe(req.commandId);
    }
...
describe('跨主体拒绝', () => {
  it('其他 subject_id 的读与写均被拒绝', async () => {
    ...
    if (!tasks.ok) expect(tasks.error.error.reasonCode).toBe('SUBJECT_MISMATCH');
    ...
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.error.error.reasonCode).toBe('SUBJECT_MISMATCH');
  });
});
