...
export class MockCoreAdapter implements ElderTerminalApi {
  ...
  private faultError(): AdapterErr | null {
    switch (this.fault) {
      ...
      case 'denied':
        return { ok: false, error: makeError('DENIED', 'SUBJECT_MISMATCH', '无权访问', false) };
      case 'failed':
        return { ok: false, error: makeError('FAILED', 'UPSTREAM_FAILED', '服务不可用', true) };
      case 'timeout':
        return { ok: false, error: makeError('FAILED', 'UPSTREAM_TIMEOUT', '服务响应超时', true) };
      default:
        return null;
    }
  }
  ...
  private check(subjectId: SubjectId): AdapterErr | null {
    if (subjectId !== this.subjectId) {
      return {
        ok: false,
        error: makeError('DENIED', 'SUBJECT_MISMATCH', '无权访问该账号', false),
      };
    }
  ...
  async chat(subjectId: SubjectId, text: string): Promise<AdapterResult<AgentResponseV1>> {
    ...
    if (this.agentFault === 'timeout') {
      return {
        ok: false,
        error: makeError('FAILED', 'UPSTREAM_TIMEOUT', '模型响应超时', true),
      };
    }
