...
class PolicyGateway:
    def authorize(self, *, actor: str, subject_id: str, capability: str, consent_scopes: list[str], idempotency_key: str) -> tuple[bool, str]:
        required = {"play_prompt":"medication_reminder","request_safety_check":"safety_check","notify_family":"family_notification","read_timeline":"daily_summary"}
        if actor != "DETERMINISTIC_PLANNER" or not subject_id.startswith("user:"): return False, "CALLER_OR_SUBJECT_DENIED"
        if not idempotency_key: return False, "IDEMPOTENCY_KEY_REQUIRED"
        if capability not in required: return False, "CAPABILITY_DENIED"
        if required[capability] not in consent_scopes: return False, "CONSENT_DENIED"
        return True, "ALLOW"
...
class DeterministicAgent:
    def __init__(self, store: EventStore, policy: PolicyGateway|None=None, executor: MockSkillExecutor|None=None, channels: ChannelMocks|None=None) -> None:
        self.store=store; self.policy=policy or PolicyGateway(); self.executor=executor or MockSkillExecutor(store); self.channels=channels or ChannelMocks(); self.response_engine=ResponseEngine()
    def run(self, *, subject_id: str, purpose: str, trigger_event_ids: list[str], channel: str, consent_scopes: list[str]) -> dict[str, Any]:
        ...
        snapshot=build_context_snapshot(subject_id=subject_id,purpose=purpose,source_event_ids=trigger_event_ids,consent_scopes=consent_scopes,consent_expires_at="2099-01-01T00:00:00+00:00",facts=[{"text":f"任务 {task['task_id']} 状态：DUE。","source_refs":trigger_event_ids}]); self.store.save_context_snapshot(snapshot)
        ...
        allowed,reason=self.policy.authorize(actor="DETERMINISTIC_PLANNER",subject_id=subject_id,capability=capability,consent_scopes=consent_scopes,idempotency_key=intent["idempotency_key"])
