...
export function fixtureSmokeGasAlert(): AlertViewV1 {
  return makeAlert({ kind: 'SMOKE_GAS', safety_level: 'S0', status: 'OPEN' });
}

export function fixtureCriticalSmokeGasAlert(): AlertViewV1 {
  return makeAlert({ kind: 'SMOKE_GAS', safety_level: 'S-1', status: 'OPEN' });
}

/** 安全告警：家庭端契约只定义 SMOKE_GAS，S-1 与 S0 均为不可忽略事件。 */
export function fixtureAlerts(): AlertViewV1[] {
  return [fixtureSmokeGasAlert(), fixtureCriticalSmokeGasAlert()];
}
...
export function fixtureAgentResponse(): AgentResponseV1 {
  return makeAgentResponse({
    ...
    facts: [
      {
        text: '上午的服药提醒已由系统生成。',
        source_refs: ['evt-medication-001'],
      },
    ],
    fallback: 'NONE',
  });
